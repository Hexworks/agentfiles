...
func TestProjectPathCannotBeSharedAcrossProfiles(t *testing.T) {
	...
	if _, addErrs := svc.AddProject("first", "Repo", projectPath, []agent.Agent{agent.Codex}, nil); len(addErrs) > 0 {
		t.Fatalf("first add: %v", addErrs)
	}

	_, addErrs := svc.AddProject("second", "Repo2", projectPath, []agent.Agent{agent.Codex}, nil)
...
func TestProjectPathCannotBeSharedWithinSameProfile(t *testing.T) {
	...
	projectPath := filepath.Join(root, "repo")
	if _, addErrs := svc.AddProject("personal", "First", projectPath, []agent.Agent{agent.Codex}, nil); len(addErrs) > 0 {
		t.Fatalf("first add: %v", addErrs)
	}

	_, addErrs := svc.AddProject("personal", "Second", projectPath, []agent.Agent{agent.Codex}, nil)
...
func TestPathOwnershipIsEnforcedInsideStore(t *testing.T) {
	...
	projectPath := filepath.Join(root, "repo")
	if _, addErrs := svc.AddProject("first", "Owner1", projectPath, []agent.Agent{agent.Codex}, nil); len(addErrs) > 0 {
		t.Fatalf("seed first: %v", addErrs)
	}

	_, addErrs := svc.AddProject("second", "Owner2", projectPath, []agent.Agent{agent.Codex}, nil)
	if len(addErrs) == 0 {
