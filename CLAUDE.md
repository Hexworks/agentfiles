...
- `utils` — shared path/IO/hashing helpers and small generic utilities (e.g. `Deduplicate`). Owns the **validated, versioned JSON persistence boundary**: `ReadJSON`/`WriteJSON`/`WriteJSONMode`/`WriteJSONAtomic` are generic over the `Persisted[T]` constraint (`*T` with `Migrate()`+`Validate()`), so every load/save runs migrate-then-validate — validation cannot be bypassed (a non-`Persisted` type is a compile error). Missing `version` = legacy sentinel (stamped up); version newer than the build knows = `errs.NewerSchemaVersionError`. See ADR 0022; the only escape hatch is `internal/migrate` reading pre-versioning DTOs raw.
