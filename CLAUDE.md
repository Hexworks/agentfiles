...
- `render` — **read-only**. Builds desired files from profile+project. Dispatch is a global strategy table keyed by the typed `(agent.Agent, asset.Type)` pair (`strategy.go`/`strategies.go`) — a map lookup, never a type switch; a missing pair accumulates `UnsupportedRenderingError`. Each strategy owns **both** directions: `Render` (forward) and `Reverse` (repo path → asset source, for Adopt), so forward and reverse cannot drift; a lossy strategy (cursor flat-file skill) reports `ok=false`. `render.ProjectPlan.ReverseLookup` (`reverse.go`) is the single reverse entry point `sync` delegates to. `agents_doc` renders per-agent (`CLAUDE.md` for claude-code, `AGENTS.md` otherwise). Calls `surfaces.IsAllowed` to gate projection targets against the safety fence (`AGENTS.md`, `CLAUDE.md`, `.claude`, `.cursor`, `.codex`, `.opencode`, `.mcp.json`). Resolves `exclusive_group` conflicts and `compatible_agents` filters — `agents_doc` now honors `compatible_agents` uniformly via the strategy dispatch gate (the pre-refactor codex arm bypassed it). See ADR 0021.
- `sync` — compares render plan vs. repo, classifies as `create`/`update`/`drift`/`delete`, writes files, and rewrites `<repo>/.agentfiles/state.json` (hashes of managed files). The repo→asset reverse mapping is not re-derived here; both Adopt call sites delegate to `render.ProjectPlan.ReverseLookup` (ADR 0021). Imported as `llmsync` in `internal/app` to avoid clashing with stdlib `sync`.
...
- `utils` — shared path/IO/hashing helpers and small generic utilities (e.g. `Deduplicate`). Owns the **validated, versioned JSON persistence boundary**: `ReadJSON`/`WriteJSON`/`WriteJSONMode`/`WriteJSONAtomic` are generic over the `Persisted[T]` constraint (`*T` with `Migrate()`+`SchemaVersion()`+`Validate()`), so every load/save runs migrate-then-reject-newer-then-validate — validation cannot be bypassed (a non-`Persisted` type is a compile error). Missing `version` = legacy sentinel (stamped up); version newer than the build knows = `errs.NewerSchemaVersionError`, raised once at the boundary from `SchemaVersion()` so per-type `Validate()` is domain-shape-only. See ADR 0022; the only escape hatch is `internal/migrate` reading pre-versioning DTOs raw.
