...
- `settings` — persistent user-preferences store at `~/.agentfiles/settings.json` (schema `{version:1, git:{enabled}}`). Same shape as `projectstore` / `registry`. Introduced by ADR 0019 so the git-integration toggle survives across launches; missing file → defaults with no error.
- `git` — narrow wrapper around the `git` binary via `os/exec` (per `docs/guidelines/external_tools.md`). Exposes `Detect(dir)`, `BinaryAvailable()`, and `Repo.Commit(pathspec, msg) (shortSHA, error)`. Consumed only by `internal/app` through the `GitCommitter` interface seam.
